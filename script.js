...
function validatePassportExpiry(expiryDate) {
    if (!expiryDate) return { valid: true, message: "" };
    ...
    if (expiry <= sixMonthsFromNow) {
        return {
            valid: false,
            message: "⚠️ يجب أن يكون الجواز صالح لأكثر من 6 أشهر\nPassport must be valid for more than 6 months"
        };
    }
    return { valid: true, message: "" };
}
...
function validateHeight(height) {
    if (!height) return { valid: true, message: "" };

    if (parseInt(height) < 150) {
        return {
            valid: false,
            message: "⚠️ الطول لا يجب أن يقل عن 150 سم\nHeight must not be less than 150 cm"
        };
    }
    return { valid: true, message: "" };
}
...
function validateWeight(weight) {
    ...
    if (parseInt(weight) > 80) {
        return {
            valid: false,
            message: "⚠️ الوزن لا يجب أن يزيد عن 80 كجم\nWeight must not exceed 80 kg"
        };
    }
    return { valid: true, message: "" };
}
...
function updatePreview() {
    ...
    // Contract is always fixed
    const contract = "2 Years";
    const contractAr = "سنتان";
    ...
    // Validate passport expiry (must be > 6 months from today)
    const passportValidation = validatePassportExpiry(expiryDate);
    if (!passportValidation.valid) {
        alert(passportValidation.message);
        return;
    }

    // Validate height (must be >= 150 cm)
    const heightValidation = validateHeight(height);
    if (!heightValidation.valid) {
        alert(heightValidation.message);
        return;
    }

    // Validate weight (must be <= 80 kg)
    ...
    // Contract is always fixed to 2 Years / سنتان
    document.getElementById("previewContract").innerHTML =
        "مدة العقد: " + contractAr;

    document.getElementById("previewMobile").innerHTML =
        "رقم الهاتف: " + (mobile || "-");

    // Photo top section (name, profession, nationality, salary)
    document.getElementById("previewPhotoName").innerText = name || "-";
    document.getElementById("previewPhotoJob").innerText =
        "المهنة: " + (profession || "-");
    document.getElementById("previewPhotoNationality").innerText =
        "الجنسية: " + (nationalityMap[nationality] || nationality || "-");
    document.getElementById("previewPhotoSalary").innerText =
        "الراتب: " + (salary || "-") + " ريال";

    // Photo bottom section (passport info)
    document.getElementById("previewPhotoPassport").innerHTML =
        "رقم الجواز: " + (passportNumber || "-");
    document.getElementById("previewPhotoIssue").innerHTML =
        "تاريخ الإصدار: " + (issueDate || "-");
    document.getElementById("previewPhotoExpiry").innerHTML =
    ...
    // Experience period: display as "المدة: X سنة"
    let periodDisplay = "-";
    if (period && parseInt(period) > 0) {
        periodDisplay = period + " سنة";
    }

    document.getElementById("previewExperience").innerHTML =
        "الدولة: " + (country || "-") + "<br>" +
        "المدة: " + periodDisplay + "<br>" +
        "الوظيفة: " + (position || "-") + "<br>" +
        "خبرات أخرى: " + (otherExperience || "-");

    document.getElementById("previewSkills").innerHTML =
        "<div>التنظيف: " + (levelMap[cleaning] || cleaning) + "</div>" +
        "<div>الطبخ: " + (levelMap[cooking] || cooking) + "</div>" +
        "<div>الطبخ العربي: " + (levelMap[arabicCooking] || arabicCooking) + "</div>" +
        "<div>الغسيل: " + (levelMap[washing] || washing) + "</div>" +
        "<div>الكوي: " + (levelMap[ironing] || ironing) + "</div>" +
        "<div>رعاية الرضع: " + (levelMap[babysitting] || babysitting) + "</div>" +
        "<div>رعاية الأطفال: " + (levelMap[childrenCare] || childrenCare) + "</div>" +
        "<div>تعليم الأطفال: " + (levelMap[tutoring] || tutoring) + "</div>" +
        "<div>رعاية كبار السن: " + (levelMap[disabledCare] || disabledCare) + "</div>";
...
// Auto-validate age on date change
